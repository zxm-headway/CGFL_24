...
def parse_args(args=None):
    ...
    parser.add_argument('--dataset', type=str, default='post')
    ...
    parser.add_argument("--corenlp", default='./stanford-corenlp-4.5.6/stanford-corenlp-4.5.6/',)
    parser.add_argument('--thres', default=0.2, type=float, help="the threshold of semantic graph")
...
def gen_corpus(dataset):
    ...
    return shuffle_doc_name_list, shuffle_doc_words_list, train_ids,val_ids, test_ids, word_doc_freq, word_id_map, id_word_map, vocab, labels, label_list
...
def main(args):
    ...
    name, corpus, train_ids, val_ids,test_ids, word_doc_freq, word_id_map, id_word_map, vocab, labels, label_list = gen_corpus(args.dataset)
    data = [train_ids, val_ids,test_ids, corpus, labels, vocab, word_id_map, id_word_map, label_list]
    json.dump(data, open('./data/{}_data.json'.format(args.dataset),'w'))
    num_labels = len(label_list)
    row_tfidf, col_tfidf, weight_tfidf = gen_tfidf(corpus, word_id_map, word_doc_freq, vocab, len(train_ids)+len(val_ids))
    if args.gen_seq:
        pmi_dict, seq_adj, row, col = gen_seq(corpus, len(train_ids)+len(val_ids), len(test_ids), args.window_size, word_id_map, row_tfidf, col_tfidf, weight_tfidf, vocab)
        f = open('./data/{}.seq_adj'.format(args.dataset), 'wb')
        pkl.dump(seq_adj, f)
        f.close()
    if args.gen_syn:
        syn_adj = gen_syn(corpus, nlp, row_tfidf, col_tfidf, weight_tfidf, word_id_map, len(train_ids)+len(val_ids)+len(vocab)+len(test_ids), len(train_ids)+len(val_ids))
        f = open('./data/{}.syn_adj'.format(args.dataset), 'wb')
        pkl.dump(syn_adj, f)
        f.close()
    if args.gen_sem:
        valid_size = len(val_ids)
        train_size = len(train_ids)
        print(train_size, valid_size)
        sem_adj = gen_sem(args, corpus, word_id_map, row_tfidf, col_tfidf, weight_tfidf, args.thres, train_size, valid_size, labels, num_labels, len(train_ids)+len(val_ids)+len(vocab)+len(test_ids),device)
